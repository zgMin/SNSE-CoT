'''
Some parameters have been added to control the use of negative sample generation and contrastive learning
'''
import os
import numpy as np
import torch
...
def parse_args():
    ...
    parser.add_argument('--enhance_LE', action='store_true', help='use contrastive loss')
...
def T5Trainer(
    ...
    val_qids = qids['val']
    output_data=None
    if args.enhance_LE is True and args.prompt_format == 'QCM-LE':
        if not os.path.exists('soft_negations'):
            os.mkdir('soft_negations')
            soft_data = Softneg(
                problems,
                train_qids,
                args.output_len,
            ...
            output_data = {
                "negs": soft_data.generate(num=args.epoch),
            }
    ...
    # if args.only is True and args.cot_encoder is True:
    #     for p in model.parameters() or model.module.model_one.parameters():
    #         p.requires_grad = False
    #     for p in model.correctencoder.parameters() or model.module.model_one.correctencoder.parameters()\
    #             or model.correctencoder.parameters() or model.module.model_one.correctencoder.parameters():
    #         p.requires_grad = True
    datacollator = DataCollatorForSeq2Seq(tokenizer)
    if args.enhance_LE and args.prompt_format == 'QCM-LE':
        model.set_enhance_LE(args.enhance_LE,alpha = args.alpha, beta = args.beta, lambda_ = args.lambda_)
    ...
    model.set_enhance_LE(False)

    test_set.set_config(enhance_LE=False)
